...
class GraafMetTakdata: public virtual Graaf<RT>{
	private: bool isMogelijk(int start,int stop, vector< vector<int> > tab) {
		const int n = Graaf<RT>::knopen.size();
		//2gevallen
		
		//Geval 1: enige verbinding die overblijft
		//bestemming zoeken
		Knoop k = Graaf<RT>::knopen[start];
		Knoop::iterator it = k.begin();
		//mogelijke tellen	
		int mog = 0;
		for (int i =0 ; i < n ; i++){
			mog += tab[start][i];
		}
		//als enkel verbindingen naar stop knoop => naar stop knoop
		if (mog == tab[start][stop]){
			return true;
		}else{		

			//2.a) aantal knopen bereikbaar
			int count1= 0;
			for (int i =0 ; i < n ; i++){
				if (tab[start][i] > 0){
					count1++;                                    
...
public:
	...
	//Eulerreeks
	vector<int> geefEulerReeks(int m){
	    //init
	    const int n = Graaf<RT>::knopen.size();
	    cout << "n: " << n << endl;
		int aantal=1;
	    ...
	    vector<int> v(aantal); //of aantal meegeven
	
		//matrix fixen
	    vector<vector<int> > hulp;
		for (int i = 0 ; i< n ; i++){
			vector<int> h(n);
			hulp.push_back(h);
			for (int j = 0 ; j < n ; j++){
		...
		//beginnen in 1e knoop => alle verbindingen m keer afgaan
		Knoop k = Graaf<RT>::knopen[0];
		int vorig = 0;
		
		for (int i = 0 ; i < aantal ; i++){
			//bestemming zoeken
			k = Graaf<RT>::knopen[vorig];
			Knoop::iterator it = k.begin();
			
			while (it != k.end() && !isMogelijk(vorig,it->first,hulp) ){
				it++;
			}
			...
			//aantal verbindingen verminderen
			hulp[vorig][it->first]--;
