...
int main()
{
	...
	int van;
	int naar;
	ifstream myfile ("bipartitegraaf.txt");
	if (myfile.is_open())
	{
		getline(myfile, line);
		GraafMetTakdata<GERICHT, int> graaf(atoi(line.c_str()));
		int p = graaf.voegKnoopToe();
		int v = graaf.voegKnoopToe();
		...
		while(!myfile.eof())
		{
			...
			if(graaf.geefTakdata(p, van) == 0)
				graaf.voegVerbindingToe(p, van, 1);
			if(graaf.geefTakdata(naar, v) == 0)
				graaf.voegVerbindingToe(naar, v, 1);
		}
		myfile.close();
		
		GraafMetTakdata<GERICHT, int>f;
		f.wordMaxStroomVan(graaf, 1024, 1025);
		//cout << "RESULTAAT:\n" << f;
		cout << f.berekenMaxStroomVan(1024);
